...
import { renderPostContent } from '@/lib/render-post';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { title: article.title, description: article.description, url, type: 'article', publishedTime: article.date, images: [{ url: `${url}/opengraph-image`, width: 1200, height: 630, alt: `${article.title}${article.subtitle ? ` — ${article.subtitle}` : ''}, ${article.dateLabel}, ${article.readMinutes} min read` }] },
    ...
    <main className="min-h-screen">
      ...
      <article className="mx-auto max-w-2xl px-5 pb-24 pt-8 sm:px-6 sm:pt-16">
        ...
        <header className="mt-12 border-b border-border pb-8 sm:mt-16 sm:pb-10">
          ...
          {article.subtitle && <p className="mt-5 text-base leading-relaxed text-muted">{article.subtitle}</p>}
          <p className={`${article.subtitle ? 'mt-2' : 'mt-5'} text-sm text-muted`}><time dateTime={article.date}>{article.dateLabel}</time><span aria-hidden="true"> · </span>{article.readMinutes} min read</p>
          {article.githubUrl && (
            <a
        ...
        <div className={`article-prose pt-8 sm:pt-10${article.content ? ' post-body' : ''}`}>{article.content ? renderPostContent(article.content) : <JevCaseStudy />}</div>
